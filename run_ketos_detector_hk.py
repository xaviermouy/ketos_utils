...
def decimate(
    ...
    if sampling_rate_hz <= audio_data.file_sampling_frequency:
        ...
        cutoff_frequencies = [20,800] #params["bandpass_filter_fc"]
        #audio_data.filter(filter_type, cutoff_frequencies, order=params["filter_order"], verbose=True)
        audio_data.filter(filter_type, cutoff_frequencies, order=8, verbose=True)
        # decimate
        audio_data.decimate(sampling_rate_hz)
        # normalize
    ...
    for idx, file in enumerate(files):
        try:
            ...
            if os.path.exists(out_file_nc + ".nc") is False:
                ...
                detec.insert_values(audio_channel=args.channel)
                detec.data.audio_channel=args.channel
